...
} from '../cli-providers/mcp/permission-server-entry';
 ...
 * 已翻译的 CC 值也原样返回，供 kernel 的 pending-mode 路径安全复用。
 *   gated        → 'default'           每个工具都走闸
 *   autoEdits    → 'acceptEdits'       自动放行编辑
 *   planning     → 'plan'              只规划不执行
 *   unrestricted → 'bypassPermissions' 绕过闸
 */
export function toCcPermissionMode(mode: PermissionMode | CcPermissionMode): CcPermissionMode {
  switch (mode) {
    case 'gated':
      return 'default';
    case 'autoEdits':
...
/** Exact stdin payload used by stream-json and persistent CC transports. */
...
/** Persistent stream-json variant: reuse one-shot capability flags, remove the
 * positional prompt, and let the caller send the structured stdin payload. */
export function buildCcPersistentArgs(
  ...
  const oneShot = buildCcArgs(req, projectRoot, sessionArgs, toCcPermissionMode(permissionMode));
 ...
 * `permissionMode` 缺省时先用 `req.permissionMode`，再用
 * {@link CC_DEFAULT_PERMISSION_MODE};传入则覆盖。这样漏传独立档位参数时不会
 * 静默落到某个旧的宽松档位，翻译只在本 profile 内发生。
 */
export function buildCcArgs(
  req: TurnRequest,
  projectRoot: string,
  sessionArgs: string[],
  permissionMode: PermissionMode | CcPermissionMode = req.permissionMode ?? CC_DEFAULT_PERMISSION_MODE,
  bootstrapContext = false,
): string[] {
  const ccPermissionMode = toCcPermissionMode(permissionMode);
  ...
  // 新建私有 session 时引入宿主选择的上下文；resume 时不重复灌入。
  ...
  return [
    ...
    '--permission-mode', ccPermissionMode,
    ...hermeticArgs,
    ...hookSettingsArgs,
    ...mcpArgs,
    ...toolPolicyArgs,
...
export function buildMcpArgs(
  ...
  if (permSid) {
    ...
    const permissionServer = resolvePermissionServerLaunch(
      resolvePath(import.meta.dirname, '../cli-providers/mcp/permission-server.mjs'),
    );
    mcpServers.forgeax = {
  ...
  if (req.tools.length > 0) {
    ...
    // 给 fxt server。The shared runtime set tracks the fxt server's actual
    ...
    // Native project tools are allowed under their real MCP server name. All
    // other tools use the fxt namespace and therefore cross the host bridge.
