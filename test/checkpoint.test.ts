...
describe("SnapshotStore", () => {
  ...
  test("restore:缺失或损坏 blob 时 fail-closed,不发生部分写入/删除", async () => {
    write("keep.ts", "v1\n");
    const store = new SnapshotStore(storeDir);
    const m = await store.snapshot(gameDir);
    write("keep.ts", "current\n");
    write("remove-on-restore.ts", "must-stay\n");
    write("new.ts", "must-stay\n");

    const blobPath = join(store.storeRoot, "blobs", m.files["keep.ts"]!.h.slice(0, 2), m.files["keep.ts"]!.h);
    rmSync(blobPath);
    const res = await store.restore(gameDir, m);
    expect(res.missingBlobs).toEqual([{ path: "keep.ts", hash: m.files["keep.ts"]!.h }]);
    expect(res.written).toEqual([]);
    expect(res.deleted).toEqual([]);
    expect(readFileSync(join(gameDir, "keep.ts"), "utf-8")).toBe("current\n");
    expect(existsSync(join(gameDir, "remove-on-restore.ts"))).toBe(true);
    expect(existsSync(join(gameDir, "new.ts"))).toBe(true);

    // Recreate the missing blob, then corrupt it. The same fail-closed contract
    // applies to checksum mismatch, not just an absent path.
    writeFileSync(blobPath, "wrong\n");
    const corrupt = await store.restore(gameDir, m);
    expect(corrupt.corruptBlobs).toEqual([{ path: "keep.ts", hash: m.files["keep.ts"]!.h }]);
    expect(corrupt.written).toEqual([]);
    expect(corrupt.deleted).toEqual([]);
  });

  test("session-scoped GC preserves another session's manifest/blob in a shared store", async () => {
    write("a.ts", "session-a\n");
    const store = new SnapshotStore(storeDir);
    const sessionA = await store.snapshot(gameDir, { sid: "session-a" });
    write("a.ts", "session-b\n");
    const sessionB = await store.snapshot(gameDir, { sid: "session-b" });
    const sessionBBlob = sessionB.files["a.ts"]!.h;

    const gc = await store.collectGarbage(new Set([sessionA.id]), { ownerSid: "session-a" });

    expect(gc.deletedManifests).not.toContain(sessionB.id);
    expect(gc.deletedBlobs).not.toContain(sessionBBlob);
    expect(existsSync(join(store.storeRoot, "manifests", `${sessionB.id}.json`))).toBe(true);
    expect(existsSync(join(store.storeRoot, "blobs", sessionBBlob.slice(0, 2), sessionBBlob))).toBe(true);
  });

  test("manifest version rejects unknown schema and GC removes unreachable CAS objects", async () => {
    write("a.ts", "a\n");
    const store = new SnapshotStore(storeDir);
    const keep = await store.snapshot(gameDir, { sid: "session-a" });
    write("a.ts", "b\n");
    const orphan = await store.snapshot(gameDir, { sid: "session-a" });
    const orphanBlob = orphan.files["a.ts"]!.h;
    const manifestPath = join(store.storeRoot, "manifests", `${orphan.id}.json`);
    const raw = JSON.parse(readFileSync(manifestPath, "utf-8"));
    raw.version = 999;
    writeFileSync(manifestPath, JSON.stringify(raw));
    expect(store.loadManifest(orphan.id)).toBeNull();
    expect(store.loadManifestChecked(orphan.id).failure?.kind).toBe("incompatible");
    // Restore the orphan manifest to a valid form so GC can prove both object
    // classes are collected from a valid but unreachable checkpoint.
    raw.version = 1;
    writeFileSync(manifestPath, JSON.stringify(raw));
    const gc = await store.collectGarbage(new Set([keep.id]), { ownerSid: "session-a" });
    expect(gc.deletedManifests).toContain(orphan.id);
    expect(gc.deletedBlobs).toContain(orphanBlob);
