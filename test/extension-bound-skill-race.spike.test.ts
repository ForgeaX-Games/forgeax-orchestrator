/**
 * Doc 14 §4 spike — extension-bound skill turn-snapshot race.
 *
 * The audit asked: when a skill calls `ctx.callTool()` and, between two such
 * calls, the user closes/reloads the extension that owns the underlying tool,
 ...
 *      to a snapshot that no longer contains the host extension, a fresh
 ...
 *      finish replaying a recorded sequence even if the extension bounces
 *      mid-turn. Test #3 below pins the desired-future shape so the loop
 *      can close once the snapshot capture lands.
 *
 * Resolution per [[14-resolutions]] §extension-race: the race is real but
...
const TMP = `/tmp/forgeax-extension-race-${process.pid}`;
const ROOT_A = join(TMP, 'A');
const ROOT_B = join(TMP, 'B');

async function buildSnapshot(rootDir: string, includeHost: boolean) {
  // Host extension contributes a tool the skill will call.
  if (includeHost) {
    const host = join(rootDir, 'user', 'host');
    mkdirSync(join(host, 'server'), { recursive: true });
    writeFileSync(
      join(host, 'forgeax-extension.json'),
      JSON.stringify({
        schemaVersion: 2,
        version: '0.1.0',
        id: '@x/host',
        displayName: { zh: 'host', en: 'host' },
    ...
    mkdirSync(join(host, 'schemas'), { recursive: true });
    writeFileSync(join(host, 'schemas', 'args.json'), '{"type":"object"}');
    writeFileSync(join(host, 'schemas', 'returns.json'), '{"type":"object"}');
    writeFileSync(
      join(host, 'server', 'handlers.ts'),
      `export const tools = { 'race.ping': async () => ({ pong: true }) };\n`,
    );
  }

  // Skill plugin always present — it's the caller, not the swapped piece.
  const sk = join(rootDir, 'user', 'skill');
...
describe('Doc 14 §4 spike — extension-bound skill turn-snapshot race', () => {
  it('baseline: with host extension present, skill completes both calls', async () => {
  ...
  it('public callTool also fails not_found after extension drop (proves snap re-read per call)', async () => {
    const withWb = await buildSnapshot(ROOT_A, true);
    const withoutWb = await buildSnapshot(ROOT_B, false);
